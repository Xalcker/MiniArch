...
El archivo `.env` tiene prioridad sobre los valores por defecto del script. Si no existe, se usan los valores predeterminados; sin embargo, `KIOSK_PASSWORD` no tiene un valor seguro por defecto y la validación detendrá la instalación hasta que lo definas.
...
# Conserva las comillas para contraseñas con espacios, #, $, &, etc.
KIOSK_PASSWORD="mi password #seguro $literal & fuerte"
```
